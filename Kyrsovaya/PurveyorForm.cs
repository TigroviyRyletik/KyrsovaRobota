...
namespace Kyrsovaya
{
    public partial class PurveyorForm : Form
    {
        ...
        {
            InitializeComponent();
            searchCriteriaComboBox.Items.AddRange(new string[] { "Назва", "Номер телефону", "ID" });
        ...
        public PurveyorForm(string role)
        {
            InitializeComponent();
            userRole = role;
        ...
        private void FillEmployeeDataGridView()
        {
            ...
            try
            {
                string query1 = "SELECT * FROM purveyor";
        ...
        private void addButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                MySqlCommand command = new MySqlCommand("INSERT INTO purveyor (PURVEYOR_NAME, CONTACT_INFORMATION) VALUES (@name, @contact)", db.getConnection());
                command.Parameters.AddWithValue("@name", textBox2.Text); // textBox2 - Назва
                command.Parameters.AddWithValue("@contact", textBox3.Text); // textBox3 - Номер телефону
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                MySqlCommand command = new MySqlCommand("UPDATE purveyor SET PURVEYOR_NAME = @name, CONTACT_INFORMATION = @contact WHERE ID_PURVEYOR = @id", db.getConnection());
                command.Parameters.AddWithValue("@name", textBox2.Text); // textBox2 - Назва
                command.Parameters.AddWithValue("@contact", textBox3.Text); // textBox3 - Номер телефону
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                MySqlCommand command = new MySqlCommand("DELETE FROM purveyor WHERE ID_PURVEYOR = @id", db.getConnection());
        ...
        private void searchButton_Click(object sender, EventArgs e)
        {
            ...
            string query = "SELECT * FROM purveyor WHERE ";

            switch (searchCriteria)
            {
                case "Назва":
                    query += "PURVEYOR_NAME LIKE @searchText";
                    break;
                case "Номер телефону":
                    query += "CONTACT_INFORMATION LIKE @searchText";
                    break;
                case "ID":
